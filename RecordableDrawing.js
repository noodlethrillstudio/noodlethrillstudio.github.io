...
RecordableDrawing = function (canvasId)
{
	...
	this.bgColor = window.getComputedStyle(document.querySelector('.drawing-board')).backgroundColor;
	this.currentLineWidth = 5;
	this.drawingColor = "#000000";
	var pauseInfo = null;
	

	this.canvas = document.getElementById(canvasId); // Assuming canvasId is provided and valid
	this.ctx = this.canvas.getContext('2d'); // Get 2D context for drawing
	...
	this.setColor = function (color)
	{
		...
		self.actions.push(colorAction);
                console.log("set color function");
		if (self.currentRecording != null)
...
Recording = function (drawingArg)
{
	...
	this.timeInterval = 0; //0 miliseconds
	this.currTime = 0;
	this.started = false;
...
Action = function()
{
	...
	this.x = 0;
	this.y = 0;
	this.iM = false;  //stands for isMovable, needed to be shorter for json efficiency. 
	
	if (arguments.length > 0)
	{
		self.aT = arguments[0]; //actionType, see above
	}
...
//These are the individual points saved to the JSON

Point = function (argX,argY,typeArg)
{
	var self = this;
	this.ty = typeArg; //0 - moveto, 1 - lineto. ty = type.
